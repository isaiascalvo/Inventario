...
namespace Application.Controllers
{
    ...
    {
        private readonly IPregnancyUseCases _pregnancyUseCases;
        private readonly IMapper _mapper;

        public PregnancyController(IPregnancyUseCases pregnancyUseCases, IMapper mapper)
        {
        ...
        {
            IEnumerable<PregnancyDto> pregncnayDto = await _pregnancyUseCases.GetAll();
            IEnumerable<PregnancyViewModel> pregnancyVM = _mapper.Map<IEnumerable<PregnancyDto>, IEnumerable<PregnancyViewModel>>(pregncnayDto);
        ...
        public async Task<IActionResult> GetOne(Guid pregnancyId)
        {
            try
            {
                ...
                var pregnancyVM = _mapper.Map<PregnancyDto, PregnancyViewModel>(pregnancyDto);
                return Ok(pregnancyVM);
            }
        ...
        {
            var pregnancyForCrationDto = _mapper.Map<PregnancyForCreationViewModel, PregnancyForCreationDto>(pregnancyForCrationVM);
        ...
        public async Task<IActionResult> Update(Guid pregnancyId, [FromBody] PregnancyViewModel pregnancyVM)
        {
            try
            {
                var pregnancyDto = _mapper.Map<PregnancyViewModel, PregnancyDto>(pregnancyVM);
